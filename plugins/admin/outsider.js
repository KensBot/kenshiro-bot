let handler = async (m, {
     ...
     participants
      ...
      try {
         let member = participants.filter(v => !v.admin).map(v => v.id).filter(v => !v.startsWith('62') && v != clips.decodeJid(clips.user.jid))
         if (!args || !args[0]) {
...
handler.help = ['outsider'].map(v => v + " *(options)*")
handler.tags = ['admins']
handler.command = ['outsider']
handler.admin = true
