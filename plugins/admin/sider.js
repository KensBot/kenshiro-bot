let handler = async (m, {
     ...
     isBotAdmin
      ...
      try {
         const member = participants.filter(v => v.admin == null).map(v => v.id).filter(v => v != clips.decodeJid(clips.user.jid))
...
handler.help = ['sider'].map(v => v + " *(options)*")
handler.tags = ['admins']
handler.command = ['sider']
handler.group = true
handler.admin = true
